...
console.log('Applying critical fixes to ensure visualization pages load correctly'); // Step 1: Create a public directory if it doesn't exist
...
var indexPath = path.resolve('./client/index.html');

if (fs.existsSync(indexPath)) {
  var content = fs.readFileSync(indexPath, 'utf8'); // Add proper head tags if missing
...
var envContent = "NODE_ENV=development\nVITE_APP_TITLE=DesignDataPro2\nVITE_BASE_URL=./\nVITE_SERVER_URL=http://localhost:3000/api\nVITE_PUBLIC_URL=./\n";
fs.writeFileSync('./.env', envContent, 'utf8');
console.log('✓ .env file created'); // Step 4: Update vite.config.js

console.log('Updating Vite configuration for proper asset loading...');
var viteConfigPath = path.resolve('./vite.config.ts');

if (fs.existsSync(viteConfigPath)) {
  var viteConfig = fs.readFileSync(viteConfigPath, 'utf8');
...
if (fs.existsSync(appPath)) {
  ...
  if (!appContent.includes('loader.js')) {
    ...
    appContent = appContent.replace('<Toaster position="top-right" />', '<Toaster position="top-right" />' + scriptTag);
...
//# sourceMappingURL=fix-visualizer.dev.js.map

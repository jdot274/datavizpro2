...
// Using .mjs extension for ES modules syntax

// Run this file using Node.js to execute the fixes
// Usage: node fix-visualizer.mjs
...
// Step 5: Fix ForceThreeVisualization component if needed
console.log('Checking and fixing ForceThreeVisualization component...');
const forcePath = path.resolve('./client/src/components/ForceThreeVisualization.tsx');

if (fs.existsSync(forcePath)) {
  let content = fs.readFileSync(forcePath, 'utf8');
  
  // Fix syntax errors if they exist
  if (content.includes('  };') && content.includes('  }')) {
    console.log('Fixing syntax in ForceThreeVisualization.tsx...');
    
    // Create a fixed version with consistent function declarations
    content = content.replace(/\}\s*\}\s*;/g, '  }\n}');
    content = content.replace(/\}\s*;(\s*\/\/)/g, '}\n$1');
    
    fs.writeFileSync(forcePath, content, 'utf8');
    console.log('✓ ForceThreeVisualization.tsx fixed');
  }
}

// Step 6: Create a loader script in public directory
...
// Step 7: Create simple service worker for resource caching
...
// Step 8: Update App.tsx to include resource loading script
...
try {
  ...
  console.log('Stopping any running processes on port 3000...');
  execSync('npx kill-port 3000', { stdio: 'ignore' });
  console.log('✓ Port cleared');
} catch (error) {
  console.log('Note: Could not clear port 3000, it may not be in use.');
}
